  ...
  program
    ...
    .addOption(
      new Option(
        ...
        .env("ARENASTATE_OPTIMISM_GASPRICE"))
    .option("-m, --margin-percent <percent>", "when sending max, the percentage to increase the total cost estimate by", 5)
    ...
    .action((key, addr, amount, options) => opsend(program, options, key, addr, amount));
}
...
async function opsend(program, options, key, address, amount) {
  ...
  if (amount === "max") {
    ...
    if (balanceBefore.lt(wei)) {
      out(`Sender balance ${wei2Gwei(balanceBefore)} GWEI to low to send ${wei2Gwei(wei)} @${signer.address}`);
    }
    out(`Sending: ${wei2Gwei(wei)}`)
  }

  if (!l2gaslimit) {
    populated = await limiter.populateAndEstimate(fromAddress, address, wei, undefined, {gasPrice});
  ...
  if (adjustForCosts) {
    ...
    populated = populated ?? await limiter.populateAndEstimate(fromAddress, address, wei, undefined, {gasPrice, ...populated?.tx});

    const l1DataFee = oracle.getL1Fee(ethers.utils.serializeTransaction(populated.tx));
    const l2GasCost = populated.tx.gasLimit.mul(populated.tx.gasPrice);
    const totalFee = l1DataFee.add(l2GasCost);

    if (totalFee.gte(wei)) {
      console.log(`Fee ${wei2Gwei(totalFee)} exceeds transfer amount ${wei2Gwei(wei)} GWEI`)
      process.exit(1);
    }
    ...
    const margin = wei.div(ethers.BigNumber.from(100)).mul(ethers.BigNumber.from(marginPercent));

    wei = wei.sub(totalFee);
    wei = wei.sub(margin);

    out(`Adjusted for fees with ${marginPercent}% margin`);
    out(`Fees Gwei   : ${wei2Gwei(fees.estTotalCost)} GWEI to transfer ${wei2Gwei(requestedWei)} GWEI`);
    if (options.ethFiat) {
      out(`Fees Fiat   : ${weiNumber2Eth(fees.estTotalCost) * Number(options.ethFiat)} ${options.fiatSymbol} to transfer ${wei2Eth(requestedWei) * Number(options.ethFiat)} GBP worth of ETH`);
    }
  ...
  let tx = populated?.tx
  if (!tx)
    tx = await limiter.populate(fromAddress, address, wei, undefined, {gasPrice, ...tx});
  ...
  const actualFee = wei2Gwei(result.gasUsed) * wei2Gwei(result.effectiveGasPrice);
