...
export function addOPDeposit(program) {
  program
    ...
    .description(
      "deposit ethereum ETH with the optimism L2 bridge to obtain ETH on optimism L2, defaults to testnet chain ids"
    )
    .addOption(
      new Option(
        "-U, --l1-url <l1url>",
...
export async function opdeposit(program, options, key, amountRequest) {
  ...
  if (amountRequest === "max") {
    ...
    out(
      `Deposit MAX: ${fmtWei(wei)} for @${forAddress}, L1 ${
        options.chainidL1
      } -> L2 ${options.chainidL2}`
    );
  } else {
    wei = amountRequest = ethers.utils.parseUnits(amountRequest, amountUnits);
    if (l1BalanceBefore.lt(wei)) {
      out(
        `Sender balance ${fmtWei(l1BalanceBefore)} to low to send ${wei2Gwei(
          wei
        )} @${signer.address}`
      );
      process.exit(1);
    ...
    out(
      `Deposit: ${fmtWei(wei)} for @${forAddress}, L1 ${
        options.chainidL1
      } -> L2 ${options.chainidL2}`
    );
  }
  ...
  if (adjustForCosts) {
    ...
    out(
      `Estimates: L1Gas ${estL1Gas}, L2Gas ${estL2Gas}, L1Cost ${fmtWei(
        estL1Cost
      )}, L2Cost ${fmtWei(estL2Cost)}`
    );
    ...
    if (wei.isNegative()) {
      out(
        `Insufficient balance to complete transfer, fee adjustment is greater than transfer amount`
      );
      out(
        `Balance: ${fmtWei(l1BalanceBefore)}, Requested: ${fmtWei(
          amountRequest
        )}`
      );
      process.exit(1);
  ...
  out(
    `Before      : On L1: ${fmtWei(l1BalanceBefore)}    On L2: ${fmtWei(
      l2BalanceBefore
    )}`
  );
  out(
    `Expect      : On L1: ${fmtWei(
      l1BalanceBefore.sub(amountRequest).add(margin)
    )}    On L2: ${fmtWei(l2BalanceBefore.add(wei))}`
  );
...
export function connectCrossChainMessenger(program, options, key, opts = {}) {
  ...
  return new CrossChainMessenger({
    ...
    l1SignerOrProvider: urlConnect(l1Url, {
      key,
      optimism: true,
      polling: true,
    }),
    l2SignerOrProvider: urlConnect(l2Url, {
      key,
      optimism: true,
      polling: true,
    }),
